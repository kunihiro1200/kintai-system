...
DROP INDEX IF EXISTS idx_email_history_sent_by;
...
-- 既存のポリシーを削除（存在する場合）
DROP POLICY IF EXISTS "認証済みユーザーは全ての送信履歴を閲覧可能" ON email_history;
DROP POLICY IF EXISTS "認証済みユーザーは送信履歴を作成可能" ON email_history;
