...
- [x] 33. 祝日対応スタッフ機能の実装
  - staffsテーブルにis_holiday_staffカラムを追加するマイグレーション作成
  - StaffServiceに祝日対応関連メソッドの実装
  - 祝日対応スタッフの設定・取得機能
  - _要件: 12.1, 12.3, 12.4_

- [ ]* 33.1 プロパティテスト: 祝日対応フラグの永続化
  - **プロパティ 33: 祝日対応フラグの永続化**
  - **検証対象: 要件 12.3**

- [x] 34. 祝日対応スタッフ設定APIの実装
  - PUT /api/staff/holiday-staff の実装
  - 祝日対応フラグの更新処理
  - エラーハンドリング
  - _要件: 12.3_

- [x] 35. 月間労働時間計算機能の実装
  - OvertimeCalculatorに月間労働時間計算メソッドを追加
  - 前月16日〜当月15日の期間計算ロジック
  - 期間内の勤怠記録の集計
  - _要件: 13.3_

- [ ]* 35.1 プロパティテスト: 月間労働時間の期間計算
  - **プロパティ 36: 月間労働時間の期間計算**
  - **検証対象: 要件 13.3**

- [x] 36. 祝日対応に基づく残業時間計算の実装
  - OvertimeCalculatorに祝日対応に基づく残業計算メソッドを追加
  - 祝日対応スタッフ: 10時間超過で残業
  - 祝日対応でないスタッフ: 7時間超過で残業
  - _要件: 13.1, 13.2_

- [ ]* 36.1 プロパティテスト: 祝日対応スタッフの残業時間計算
  - **プロパティ 34: 祝日対応スタッフの残業時間計算**
  - **検証対象: 要件 13.1**

- [ ]* 36.2 プロパティテスト: 祝日対応でないスタッフの残業時間計算
  - **プロパティ 35: 祝日対応でないスタッフの残業時間計算**
  - **検証対象: 要件 13.2**

- [x] 37. 月間残業時間取得APIの実装
  - GET /api/attendance/monthly-overtime の実装
  - スタッフの祝日対応状態の取得
  - 月間労働時間と残業時間の計算
  - 期間情報の返却
  - _要件: 13.1, 13.2, 13.3, 13.4_

- [x] 38. スタッフ管理画面UIの更新 - 祝日対応
  - 祝日対応スタッフの表示
  - 祝日対応状態の切り替えボタン
  - Google Sheetsからの祝日対応フラグ同期
  - _要件: 12.2, 12.3, 12.5_

- [x] 39. 勤怠履歴画面UIの更新 - 月間残業時間
  - 月間残業時間の表示
  - 祝日対応状態に基づいた残業時間の表示
  - 期間情報（前月16日〜当月15日）の表示
  - _要件: 13.4_

- [x] 40. 管理画面UIの更新 - 祝日対応と残業時間
  - 全社員サマリーに祝日対応列を追加
  - 祝日対応状態に基づいた残業時間の表示
  - _要件: 12.2, 13.5_

- [x] 41. Google連携ボタンの位置変更
  - メイン画面のGoogle連携ボタンを画面下部に移動
  - レイアウトの調整
  - _UI変更_

- [ ] 42. チェックポイント - 祝日対応機能テスト
  - すべてのテストが通ることを確認し、問題があればユーザーに質問する
...
- [ ] 43. 最終統合テスト - 祝日対応機能
  ...
  - 祝日対応に基づく残業時間の計算確認
  - UI表示の確認
  - _要件: 12.1, 12.2, 12.3, 12.4, 12.5, 13.1, 13.2, 13.3, 13.4, 13.5_
