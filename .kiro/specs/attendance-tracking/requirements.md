...
- **祝日対応スタッフ**: 祝日に出勤する可能性があるスタッフ（月10時間超過で残業時間に該当）
- **月間労働時間**: 前月16日から当月15日までの期間における総労働時間
...
**ユーザーストーリー:** 管理者として、スタッフごとに祝日対応の有無を設定したい。祝日対応スタッフとそうでないスタッフで残業時間の計算ルールが異なるため。

#### 受入基準

1. THE システム SHALL 各スタッフに祝日対応フラグを保存する
2. WHEN 管理者がスタッフ管理画面を表示した時、THE システム SHALL 各スタッフの祝日対応状態を表示する
3. WHEN 管理者がスタッフの祝日対応状態を変更した時、THE システム SHALL 変更を即座にデータベースに保存する
4. WHEN スタッフ情報を取得した時、THE システム SHALL 祝日対応フラグを含める
5. WHEN Google Sheetsからスタッフ情報を同期する時、THE システム SHALL 祝日対応フラグを取得し保存する

### 要件 13

**ユーザーストーリー:** システムとして、スタッフの祝日対応状態に基づいて残業時間を計算したい。祝日対応スタッフとそうでないスタッフで残業の基準が異なるため。

#### 受入基準

1. WHEN 祝日対応スタッフの月間労働時間を計算する時、THE システム SHALL 月10時間を超えた時間を残業時間として計算する
2. WHEN 祝日対応でないスタッフの月間労働時間を計算する時、THE システム SHALL 月7時間を超えた時間を残業時間として計算する
3. WHEN 月間労働時間を計算する時、THE システム SHALL 前月16日から当月15日までの期間を1ヶ月として扱う
4. WHEN スタッフの勤怠履歴を表示する時、THE システム SHALL 祝日対応状態に基づいた残業時間を表示する
5. WHEN 管理者が全社員サマリーを閲覧した時、THE システム SHALL 各スタッフの祝日対応状態に基づいた残業時間を表示する
