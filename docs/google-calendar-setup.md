...
1. ポップアップが表示されたら、以下をコピー:
   - **クライアントID**: `1072003145082-h32d02toreqbai6oq10d30lh6k43pa7j.apps.googleusercontent.com`
   - **クライアントシークレット**: `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`
2. 「OK」をクリック
...
## ステップ8: システム管理者の設定（Google Sheets連携用）

### 8.1 システム管理者とは

システム管理者は、Google Sheets APIアクセスのための個人Gmailトークンを提供する特別な権限を持つスタッフです。

**背景:**
- 組織アカウント（@ifoo-oita.com）ではGoogle APIが利用できない場合がある
- システム管理者の個人Gmailアカウントのトークンを使用してGoogle Sheetsにアクセス
...
2. **システム管理者を設定**
   - スタッフ一覧から、個人Gmailアカウントを持つスタッフを選択
   - 「システム管理者に設定」ボタンをクリック

3. **個人Gmailで連携**
   - システム管理者として設定されたスタッフでログイン
   - ホーム画面で「Googleカレンダーと連携」をクリック
   - **個人Gmailアカウント**を選択して連携
   - 必要な権限（カレンダー、スプレッドシート）を許可

4. **連携状態の確認**
   - スタッフ管理画面で「システム管理者ステータス」を確認
   - 「Google連携: ✓ 連携済み」と表示されればOK

### 8.3 Google Sheets同期の実行

1. スタッフ管理画面で「スプレッドシートから同期」ボタンをクリック
2. システム管理者のトークンを使用してスプレッドシートにアクセス
3. スタッフ情報が自動的に同期される

**注意事項:**
- システム管理者は1人のみ設定可能
- システム管理者のGoogle連携が切れた場合は、再度連携が必要
- スプレッドシートは、システム管理者のGoogleアカウントに共有されている必要がある
